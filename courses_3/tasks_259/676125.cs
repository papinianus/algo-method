...
namespace template
{
    ...
    public static class Solver
    {
        public static void Solve()
        {
            GetInt();
            var nums = GetInts();
            Enumerable.Range(0, nums.Length).SelectMany(x =>
                    Enumerable.Range(1, nums.Length - 1).SelectMany(y =>
                        Enumerable.Range(2, nums.Length - 2).Select(z => new[] { x, y, z })))
                .Where(x => x[2] > x[1] && x[1] > x[0])
                .Count(x => (new[] { nums[x[0]], nums[x[1]], nums[x[2]] }).Max() == nums[x[1]]).Echo();
        }
    }
