...
namespace template
{
    ...
    public static class Solver
    {
        ...
        {
            foreach (var i in Enumerable.Range(1, GetInt()))
            {
                ToFizzBuzzString(i).Echo();
            }
        }

        private static string ToFizzBuzzString(int n)
        {
            var fizzybuzzy = $"{(n % 3 == 0 ? "Fizz" : string.Empty)}{(n % 5 == 0 ? "Buzz" : string.Empty)}";
            return string.IsNullOrEmpty(fizzybuzzy) ? n.ToString() : fizzybuzzy;
        }
    }
