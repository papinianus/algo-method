...
namespace template
{
    ...
    public static class Solver
    {
        ...
        {
            GetInt();
            GetInts().Count(x => PrimeNumbers.isPrime(x)).Echo();
        }
    }
    ...
    public static class PrimeNumbers
    {
        ...
        private static List<int> nonPrimes = new List<int>() { 1 };

        public static bool isPrime(int val)
        {
            if (nonPrimes.Contains(val)) return false;
            if (primes.Contains(val)) return true;
            var upper = Math.Sqrt(val);
