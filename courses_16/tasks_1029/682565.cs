...
namespace template
{
    ...
    public static class Solver
    {
        ...
        {
            GetInt();
            var nums = GetInts().GroupBy(ident).ToDictionary(x => x.Key, x => x.Count());
            var sums = Enumerable.Repeat(0L, (int)Math.Pow(10, 5) + 1).ToArray();
            foreach (var i in Enumerable.Range(1, (int)Math.Pow(10, 5)))
            {
                sums[i] = sums[i - 1];
                var v = 0;
                if (nums.TryGetValue(i, out v))
                {
                    sums[i] += v;
                }
            }
